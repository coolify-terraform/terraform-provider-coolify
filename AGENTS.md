...
57 resources, 71 data sources, 1670+ tests (unit + acceptance), 11 CI jobs.
...
- Contract skip taxonomy (`internal/spectest/skip.go`): `internal` (FK/computed), `deferred` (public field + open issue `#N`), `n/a` (wrong surface). Never label public `$allowedFields` as internal. Schema registry Phase A: EnvironmentVariable; Phase B: Application settings + ScheduledTask; Phase C: ApplicationsController create/update `$allowedFields` (`schema_coverage*.go`). Write-path check: application env `allowed_fields` vs client input (`write_coverage_test.go`). Weekly drift issue body: `.github/contract-freshness-issue.md`. `make contract-check` runs the full coverage suite. Extract aliases private `validate*` helpers that own `$allowedFields` onto public callers (e.g. VolumeBackups `upsert` ← `validateUpsertRequest`). Extract also reads `$backupConfigFields` on DatabasesController `create_backup` / `update_backup` (#847).
...
- **Create-only optional flags** the user may omit (e.g. `coolify_github_app.is_system_wide`) use `RequiresReplaceIfConfigured()` and must **not** set `Default`. `Default` plus unconditional `RequiresReplace()` plans destroy/recreate when Coolify drifts. Identity fields with a real default (`environment_name`) may keep `Default` + `RequiresReplace()`.
...
Acceptance Tests (2 shards), Scenario Tests (3 suites), Contract Freshness (weekly only), Workflow Lint, CI (gate).
