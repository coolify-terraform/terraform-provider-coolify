...
20 resources, 19 data sources, 201 tests, 8 CI jobs.
...
- `internal/acctest/` - Shared test utilities (provider factories, mock server wrappers)
...
- 201 tests (200 unit + 1 acceptance), ~76% statement coverage
- Acceptance tests are skipped unless `TF_ACC=1` is set
- Run `go build ./...` after every edit before committing (local gate, not CI)
- Before adding a test function, grep for its name to avoid duplicates

## CI

8 GitHub Actions jobs on push to main and PRs:
Test, Lint, Format, Docs, Validate Examples, Govulncheck, Trivy, Gitleaks.
