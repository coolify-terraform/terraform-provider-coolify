...
run "update_project_description" {
  ...
  assert {
    condition     = coolify_project.platform.description == "Updated by scenario test"
    error_message = "Project description not updated: got ${coolify_project.platform.description}"
  }
}
