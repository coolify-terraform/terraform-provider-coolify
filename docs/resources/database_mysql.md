...
- `ssl_mode` (String) The SSL connection mode. Only applies when `enable_ssl` is `true`. Valid values: `REQUIRED`, `DISABLED`, `PREFERRED`, `VERIFY_CA`, `VERIFY_IDENTITY`. The Coolify public API does not accept `ssl_mode` on create or update; set it in the Coolify UI. The provider keeps the configured value.
