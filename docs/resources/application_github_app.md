...
- `docker_compose_custom_build_command` (String) Custom `docker compose` build command used when `build_pack = "dockercompose"`. Coolify validates the value as a shell-safe command (max 1000 characters; `&&` and `||` are allowed; `;`, `|`, `$`, backticks, and newlines are not). Present on every Coolify version this provider supports (v4.1.0 and later); no version gate. Deployment injects `--project-directory` when the command does not already include it. Omitting the attribute later does not clear the stored command.
- `docker_compose_custom_start_command` (String) Custom `docker compose` start command used when `build_pack = "dockercompose"`. Coolify validates the value as a shell-safe command (max 1000 characters; `&&` and `||` are allowed; `;`, `|`, `$`, backticks, and newlines are not). Present on every Coolify version this provider supports (v4.1.0 and later); no version gate. Deployment injects `--project-directory` when the command does not already include it. Omitting the attribute later does not clear the stored command.
