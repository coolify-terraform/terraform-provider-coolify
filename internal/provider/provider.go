...
type coolifyProviderModel struct { Endpoint types.String `tfsdk:"endpoint"`; Token types.String `tfsdk:"token"` }
func New(version string) func() provider.Provider { return func() provider.Provider { return &coolifyProvider{version: version} } }
func (p *coolifyProvider) Metadata(_ context.Context, _ provider.MetadataRequest, resp *provider.MetadataResponse) { resp.TypeName = "coolify"; resp.Version = p.version }
...
func (p *coolifyProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	var config coolifyProviderModel; resp.Diagnostics.Append(req.Config.Get(ctx, &config)...); if resp.Diagnostics.HasError() { return }
	endpoint := os.Getenv("COOLIFY_ENDPOINT"); if !config.Endpoint.IsNull() && !config.Endpoint.IsUnknown() { endpoint = config.Endpoint.ValueString() }
	if endpoint == "" { resp.Diagnostics.AddError("Missing Coolify Endpoint", "Set endpoint in provider block or COOLIFY_ENDPOINT env var.") }
	token := os.Getenv("COOLIFY_TOKEN"); if !config.Token.IsNull() && !config.Token.IsUnknown() { token = config.Token.ValueString() }
	if token == "" { resp.Diagnostics.AddError("Missing Coolify Token", "Set token in provider block or COOLIFY_TOKEN env var.") }
	if resp.Diagnostics.HasError() { return }
	c := client.New(endpoint, token); resp.DataSourceData = c; resp.ResourceData = c
}
func (p *coolifyProvider) Resources(_ context.Context) []func() resource.Resource {
	return []func() resource.Resource{application.NewResource, environmentvariable.NewResource, postgresql.NewResource, mysql.NewResource, mariadb.NewResource, redis.NewResource, mongodb.NewResource, privatekey.NewResource, project.NewResource, server.NewResource, service.NewResource}
}
func (p *coolifyProvider) DataSources(_ context.Context) []func() datasource.DataSource {
	return []func() datasource.DataSource{application.NewDataSource, project.NewDataSource, project.NewListDataSource, server.NewDataSource, server.NewListDataSource, privatekey.NewDataSource, team.NewDataSource}
}
