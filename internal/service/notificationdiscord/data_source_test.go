...
func TestNotificationDataSource_Read(t *testing.T) {
	t.Parallel()
	store := &mockDiscord{EventStore: notificationcommon.EventStore{DeploymentFailure: true}}
...
data "coolify_notification_discord" "test" {}
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("data.coolify_notification_discord.test", "id", "current"),
					resource.TestCheckResourceAttr("data.coolify_notification_discord.test", "deployment_failure", "true"),
				),
			},
		},
	})
