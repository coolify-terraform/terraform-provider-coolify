...
func TestNotificationDataSource_Read(t *testing.T) {
	t.Parallel()
	store := &mockSlack{EventStore: notificationcommon.EventStore{BackupFailure: true}}
...
data "coolify_notification_slack" "test" {}
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("data.coolify_notification_slack.test", "id", "current"),
					resource.TestCheckResourceAttr("data.coolify_notification_slack.test", "backup_failure", "true"),
				),
			},
		},
	})
