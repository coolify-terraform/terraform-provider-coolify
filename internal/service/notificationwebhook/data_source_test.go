...
func TestNotificationDataSource_Read(t *testing.T) {
	t.Parallel()
	store := &mockWebhook{EventStore: notificationcommon.EventStore{StatusChange: true}}
...
data "coolify_notification_webhook" "test" {}
`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("data.coolify_notification_webhook.test", "id", "current"),
					resource.TestCheckResourceAttr("data.coolify_notification_webhook.test", "status_change", "true"),
				),
			},
		},
	})
