...
func TestServerLogDrainResource_NewRelicURIOmitKeepsState(t *testing.T) {
	t.Parallel()
	const serverUUID = "aaaa0001-0001-4000-8000-000000000001"
	const newRelicURI = "https://log-api.newrelic.com/log/v1"
	store := map[string]any{
		"is_logdrain_newrelic_enabled": false,
		"is_logdrain_axiom_enabled":    false,
		"is_logdrain_custom_enabled":   false,
	...
	srv := httptest.NewServer(acctest.WithVersionEndpoint(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPatch:
			...
			if afterCreate {
				if _, sent := body["logdrain_newrelic_base_uri"]; sent {
					t.Error("omit-after-set must not PATCH logdrain_newrelic_base_uri")
				}
			}
			if body["is_logdrain_axiom_enabled"] == nil && body["is_logdrain_newrelic_enabled"] == nil && body["is_logdrain_custom_enabled"] == nil {
				t.Errorf("expected a drain enable flag in PATCH body, got %v", body)
...
resource "coolify_server_log_drain" "test" {
  ...
  is_logdrain_axiom_enabled    = false
  is_logdrain_custom_enabled   = false
  logdrain_newrelic_base_uri   = "` + newRelicURI + `"
}`,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("coolify_server_log_drain.test", "logdrain_newrelic_base_uri", newRelicURI),
