...
`,
			ExpectError: regexp.MustCompile(`MariaDB database created but refresh failed`),
		}},
	})
}
...
func TestMariadbDatabaseResource_CreateAPIError(t *testing.T) {
	t.Parallel()

	mux := http.NewServeMux()
	mux.HandleFunc("POST /api/v1/databases/mariadb", func(w http.ResponseWriter, _ *http.Request) {
...
resource "coolify_database_mariadb" "test" {
  ...
  server_uuid  = "bbbb0001-0001-4000-8000-000000000001"
				...
				ExpectError: regexp.MustCompile(`Error creating MariaDB database`),
			},
		},
	})
}
