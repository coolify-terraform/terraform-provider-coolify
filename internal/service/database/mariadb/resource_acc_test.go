...
func TestAccMariadbDatabaseResource_CRUD(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Create
			{
				Config: testAccMariadbConfig(name, serverUUID, ""),
			...
			{
				Config: testAccMariadbConfig(name, serverUUID, `description = "Updated via acc test"`),
...
func testAccMariadbConfig(name, serverUUID, extra string) string {
