...
func TestAccRedisDatabaseResource_CRUD(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Create
			{
				Config: testAccRedisConfig(name, serverUUID, ""),
			...
			{
				Config: testAccRedisConfig(name, serverUUID, `description = "Updated via acc test"`),
...
func testAccRedisConfig(name, serverUUID, extra string) string {
