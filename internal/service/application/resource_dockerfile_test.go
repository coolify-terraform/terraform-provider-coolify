...
// ---------------------------------------------------------------------------
// TestDockerfileApplicationResource_CreateAPIError
// ---------------------------------------------------------------------------

func TestDockerfileApplicationResource_CreateAPIError(t *testing.T) {
	t.Parallel()
	mux := http.NewServeMux()
		...
		Steps: []resource.TestStep{
			{
				Config: testDockerfileResourceConfig(srv.URL, `
					...
					ports_exposes       = "80"
				`),
				ExpectError: regexp.MustCompile(`Error creating dockerfile application`),
			},
		},
	})
}
