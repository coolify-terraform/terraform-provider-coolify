...
func TestHasNonDefaultAppExtendedFields_EachField(t *testing.T) {
	...
	tests := []struct {
		...
		{"IsPreviewDeploymentsEnabled", func(f *commonAppFields) { f.IsPreviewDeploymentsEnabled = boolPtr(true) }},
		{"UseBuildSecrets", func(f *commonAppFields) { f.UseBuildSecrets = boolPtr(true) }},
		{"StopGracePeriod", func(f *commonAppFields) { f.StopGracePeriod = int64Ptr(30) }},
