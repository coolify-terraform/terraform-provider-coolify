...
func flattenLimitsAndHealth(app *client.Application, f commonAppFields) {
	...
	*f.HealthCheckType = flex.StringValueOrDefault(app.HealthCheckType, defaultHealthCheckType)
	...
	// health_check_enabled has no schema Default (Coolify column default is false).
	// is_auto_deploy_enabled has Default: true. Always resolve after Create/import.
	...
	setBoolDefault(f.IsAutoDeployEnabled, app.IsAutoDeployEnabled, true)
}
...
func flattenExtendedFields(app *client.Application, f commonAppFields) {
	...
	if f.DockerComposeLocation != nil {
		flex.SetStringSeedOrClear(f.DockerComposeLocation, app.DockerComposeLocation)
	}
