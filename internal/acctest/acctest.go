...
// AccTestSkipIfCannotClearApplicationDomains skips when Coolify does not
// persist empty `domains` as a cleared FQDN (provider #647,
// coollabsio/coolify#11116). Soft-skip that ignore-empty case even when
// COOLIFY_REQUIRE_TIP_APIS=1: this is blocked-upstream, not a claimed tip
// controller. Transport errors and HTTP 5xx use accTestMissingFeature
// (hard fail under REQUIRE_TIP_APIS).
func AccTestSkipIfCannotClearApplicationDomains(t *testing.T) {
	t.Helper()
	ok, detail := coolifyClearsApplicationDomains(t)
	if !ok {
		t.Skip(detail)
	}
}

func coolifyClearsApplicationDomains(t *testing.T) (ok bool, detail string) {
	t.Helper()
	TestAccPreCheck(t)
	c := AccTestClient(t)
	serverUUID := AccTestServerUUID(t)
	ctx, cancel := context.WithTimeout(context.Background(), 45*time.Second)
	defer cancel()

	proj, app, err := createClearDomainsProbeApp(ctx, c, serverUUID)
	if err != nil {
		return clearDomainsProbeFail(t, err)
	}
	defer deleteClearDomainsProbe(c, app.UUID, proj.UUID)

	got, err := c.GetApplication(ctx, app.UUID)
	if err != nil {
		return clearDomainsProbeFail(t, err)
	}
	if strings.TrimSpace(got.Domains) == "" {
		return false, "Coolify did not persist create-time domains (proxy may not be running); cannot prove empty-domains clear"
	}

	empty := ""
	if _, err := c.UpdateApplication(ctx, app.UUID, client.UpdateApplicationInput{Domains: &empty}); err != nil {
		return clearDomainsProbeFail(t, err)
	}
	got, err = c.GetApplication(ctx, app.UUID)
	if err != nil {
	...
	if strings.TrimSpace(got.Domains) != "" {
		return false, fmt.Sprintf("Coolify PATCH domains=\"\" did not clear fqdn (still %q); tracked in #647 and coollabsio/coolify#11116", got.Domains)
	}
	return true, ""
}

func createClearDomainsProbeApp(ctx context.Context, c *client.Client, serverUUID string) (*client.Project, *client.Application, error) {
	name := RandomWithPrefix("tf-acc-clrdmn")
	domain := fmt.Sprintf("http://%s.example.com", name)
	off := false
	proj, err := c.CreateProject(ctx, client.CreateProjectInput{Name: name})
	if err != nil {
		return nil, nil, err
	}
	app, err := c.CreateDockerImageApplication(ctx, client.CreateDockerImageAppInput{
		...
		Domains:            domain,
		InstantDeploy:      &off,
		AutogenerateDomain: &off,
	})
	if err != nil {
		_ = c.DeleteProject(ctx, proj.UUID)
		return nil, nil, err
	}
	return proj, app, nil
}

func deleteClearDomainsProbe(c *client.Client, appUUID, projectUUID string) {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	if appUUID != "" {
		_ = c.DeleteApplication(ctx, appUUID)
	}
	if projectUUID == "" {
		return
	}
	for range 8 {
		err := c.DeleteProject(ctx, projectUUID)
...
func clearDomainsProbeFail(t *testing.T, err error) (bool, string) {
	t.Helper()
	if isClearDomainsProbeInfraError(err) {
		accTestMissingFeature(t, "application domains-clear probe failed: %v", err)
		return false, ""
	}
	return false, fmt.Sprintf("application domains-clear probe failed: %v", err)
}

func isClearDomainsProbeInfraError(err error) bool {
	if err == nil {
		return false
	}
	...
	// POST transport/deadline is not retried, so it has no APIStatusError.
	return true
}
